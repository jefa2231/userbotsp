"""
> Based on parallel_file_transfer.py from mautrix-telegram,
...
from telethon.tl.types import (Document, InputFileLocation,
                               ...
                               InputPeerPhotoFileLocation, TypeInputFile,
...
class DownloadSender:
    ...
    def __init__(self, sender: MTProtoSender, file: TypeLocation, offset: int,
                 limit: int, stride: int, count: int) -> None:
        self.sender = sender
...
class UploadSender:
    ...
    def __init__(self, sender: MTProtoSender, file_id: int, part_count: int,
                 big: bool, index: int, stride: int,
                 loop: asyncio.AbstractEventLoop) -> None:
        self.sender = sender
        self.part_count = part_count
        if big:
            self.request = SaveBigFilePartRequest(file_id, index, part_count,
                                                  b"")
        else:
...
class ParallelTransferrer:
    ...
    @staticmethod
    def _get_connection_count(file_size: int,
                              max_count: int = 20,
    ...
    async def _init_download(self, connections: int, file: TypeLocation,
        ...
        # The first cross-DC sender will export+import the authorization,
        # so we always create it before creating any other senders.
        self.senders = [
            await self._create_download_sender(file, 0, part_size,
                                               connections * part_size,
                                               get_part_count()),
            *await asyncio.gather(*[
                self._create_download_sender(file, i, part_size, connections *
                                             part_size, get_part_count())
                for i in range(1, connections)
            ])
        ]

    async def _create_download_sender(self, file: TypeLocation, index: int,
                                      part_size: int, stride: int,
                                      part_count: int) -> DownloadSender:
        return DownloadSender(await self._create_sender(), file,
                              index * part_size, part_size, stride, part_count)

    async def _init_upload(self, connections: int, file_id: int,
                           part_count: int, big: bool) -> None:
        self.senders = [
    ...
    async def _create_upload_sender(self, file_id: int, part_count: int,
                                    big: bool, index: int,
                                    stride: int) -> UploadSender:
        return UploadSender(await self._create_sender(),
                            file_id,
                            part_count,
                            big,
                            index,
    ...
    async def init_upload(
        ...
        connection_count = (connection_count
                            or self._get_connection_count(file_size))
        ...
        await self._init_upload(connection_count, file_id, part_count,
                                is_large)
...
async def _internal_transfer_to_telegram(
        client: TelegramClient, response: BinaryIO,
        progress_callback: callable) -> Tuple[TypeInputFile, int]:
...
async def download_file(self: TelegramClient,
